...
export const WorkAllDesc: WorkAllDescInterface = {
  ...
  quiz_me: {
    ...
    detail: {
      ...
      long_summary: [
        "이 프로젝트는 개인적으로 개발한 퀴즈 공유 서비스입니다.",
      ...
      review: [
        ...
        "처음에는 Next.js를 사용하여 프로젝트를 시작했습니다. Next.js는 SSR과 SSG 기능을 제공하여 성능 면에서 매우 유리했지만, GitHub Pages에 배포할 때 동적 라우팅 문제에 부딪히게 되었습니다. [uuid].js 파일을 통해 동적 페이지를 생성하려 했으나, GitHub Pages에서는 이 방식이 제대로 작동하지 않았습니다. 이 문제를 해결하기 위해 여러 가지 설정을 변경해 보았지만, 현존하는 Next.js의 한계로 사용자가 퀴즈를 생성할 때마다 빌드를 다시 해야 하는 방법밖에 없었습니다.",
        "결국엔 Next.js에서 React로 전환하기로 결정했습니다. React는 클라이언트 사이드에서 동적 라우팅을 쉽게 구현할 수 있는 React Router를 제공하고 경험도 있어서, 이 프로젝트의 요구사항에 더 적합하다고 판단했습니다. React로 전환한 후, React Router를 사용하여 UUID 기반의 동적 라우팅을 설정했고, 이로 인해 사용자가 생성한 퀴즈 링크의 라우팅 문제가 해결됐습니다.",
        "배포 후 스타일링이 깨지는 문제도 발생했습니다. 로컬 환경에서는 정상적으로 보이던 스타일이 GitHub Pages에 배포된 후에는 깨져 보였습니다. 이를 해결하기 위해 Tailwind CSS의 purge 배열 내용을 변경해줘서 현재 프로젝트 내에서 사용하고 있는 tailwind css 속성만 해석해서 build할 수 있도록 해주었습니다.",
