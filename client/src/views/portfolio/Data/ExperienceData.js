const ExperienceData = [
  {
    title: "동서대학교 소프트웨어학과",
    period: "2018.03 ~ 2024.02",
    location: "Busan, Korea",
    website: "https://www.dongseo.ac.kr/",
    website_n: "dongseo.ac.kr",
    desc: "동서대학교 소프트웨어학과에서 학사 학위를 취득하였습니다. 소프트웨어 개발과 관련된 다양한 프로젝트를 진행하며, 프로그래밍 언어와 데이터베이스, 네트워크, 운영체제, 알고리즘 등의 기초 지식을 습득하였습니다. 또한, 디자인과 관련된 수업을 수강하며 UI/UX 디자인에 대한 기초 지식을 습득하였습니다.",
    stack: ["UbSE Lab"],
  },
  {
    title: "R&D 전문기업 에핀(EPIN) 인턴십",
    period: "2022.06 ~ 2022.08",
    location: "Busan, Korea",
    website: "https://epin.co.kr/",
    website_n: "epin.co.kr",
    desc: 'R&D 전문기업 에핀(EPIN)에서 인턴십을 진행하였습니다. 당시 Epin에서 개발 중이었던 "테니스 대진표 자동화 프로젝트"에 참여하며, 프로젝트의 일정에 맞춰 개발을 진행하였습니다. 또한, 프로젝트에 사용된 기술 스택에 대해 학습하며, 실무에서의 개발 경험을 쌓을 수 있었습니다.',
    stack: ["동서대학교 산학협력단", "테니스 대진표 자동화 프로젝트", "Flutter", "Firebase", "Dart", "Node.js"],
  },
];

export default ExperienceData;
